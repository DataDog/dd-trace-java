 ...
 * `// tagId(...)` derivation comment), the `StringIndex.EmbeddingSupport` keyOf table, the `globalSerial`
...
object KnownTagsEmitter {
  ...
  fun emit(reg: TagRegistry, pkg: String, className: String): String {
    ...
    reg.virtuals.forEach { mk(it.name) }
    reg.stored.forEach { mk(it.name) }

    val order = reg.virtuals.map { it.name } + reg.stored.map { it.name } // stable emit order
    val b = StringBuilder()
    ...
    b.appendLine("  // ---- reserved / virtual (routed to span fields or directives; not stored) ----")
    for (v in reg.virtuals) {
      val cn = cname[v.name]!!
      b.appendLine("  static final int ${cn}_SERIAL = ${v.serial};")
      b.appendLine("  // tagId(serial=${v.serial}, intercepted=true, slot=NO_SLOT)  [${v.kind}${v.field?.let { " -> $it" } ?: ""}]  \"${v.name}\"")
      b.appendLine("  public static final long $cn = ${hex(v.id)};")
    }
    b.appendLine()

    b.appendLine("  // ---- stored (dense-store slot, or bucketed when slot=NO_SLOT) ----")
    for (t in reg.stored) {
      val cn = cname[t.name]!!
      val slot = if (t.slotted) t.slot.toString() else "NO_SLOT"
      b.appendLine("  static final int ${cn}_SERIAL = ${t.serial};")
      b.appendLine("  // tagId(serial=${t.serial}, intercepted=${t.intercepted}, slot=$slot)  <${t.required}${if (t.traceLevel) ", trace-level" else ""}>  \"${t.name}\"")
