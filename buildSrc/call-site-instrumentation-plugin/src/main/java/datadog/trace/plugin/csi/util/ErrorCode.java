...
public enum ErrorCode implements Function<Object[], String> {
  ...
  CALL_SITE_ENABLED_SHOULD_BE_STATIC_AND_PUBLIC {
    ...
    public String apply(final Object[] objects) {
      ...
      return String.format(
          "@CallSite enabled method should be static and public, received '%s'",
          method);
    }
  },
  CALL_SITE_SPI_SHOULD_BE_EMPTY {
    ...
    public String apply(final Object[] objects) {
      final Class<?> spiClass = (Class<?>) objects[0];
      return String.format(
          "@CallSite spi class should not define any methods, spi class '%s'",
          spiClass);
    }
  },
  ...
  ADVICE_METHOD_PARAM_NOT_COMPATIBLE {
    ...
    public String apply(final Object[] objects) {
      ...
      return String.format(
          "Call site @Argument with type '%s' not compatible with pointcut parameter type "
          + "'%s', found at index '%s'",
          callSite.getClassName(),
          pointcut.getClassName(),
          index);
  ...
  ADVICE_PARAMETER_THIS_DUPLICATED {
    ...
    public String apply(final Object[] objects) {
      final int index = (int) objects[0];
      return String.format(
          "Call site parameter annotated with @This is duplicated, found at index '%s'",
          index);
    }
  },
  ...
  ADVICE_PARAMETER_THIS_ON_INVOKE_DYNAMIC {
    ...
    public String apply(final Object[] objects) {
      final int index = (int) objects[0];
      return String.format(
          "Call site parameter can't be annotated with @This for invoke dynamic instructions,"
          + " found at index '%s'",
          index);
    }
  },
  ADVICE_PARAMETER_RETURN_DUPLICATED {
    ...
    public String apply(final Object[] objects) {
      final int index = (int) objects[0];
      return String.format(
          "Call site parameter annotated with @Return duplicated, found at index '%s'",
          index);
    }
  },
  ...
  ADVICE_PARAMETER_INVOKE_DYNAMIC_CONSTANTS_NON_AFTER_ADVICE {
    ...
    public String apply(final Object[] objects) {
      final int index = (int) objects[0];
      return String.format(
          "Call site parameter annotated with @InvokeDynamicConstants should only be used on "
          + "@After call sites, found at index '%s'",
          index);
    }
  },
  ADVICE_PARAMETER_INVOKE_DYNAMIC_CONSTANTS_ON_NON_INVOKE_DYNAMIC {
    ...
    public String apply(final Object[] objects) {
      final int index = (int) objects[0];
      return String.format(
          "Call site parameter annotated with @InvokeDynamicConstants should target only "
          + "invoke dynamic instructions, found at index '%s'",
          index);
    }
  },
  ...
  ADVICE_PARAMETER_ON_INVOKE_DYNAMIC {
    ...
    public String apply(final Object[] objects) {
      final int index = (int) objects[0];
      return String.format(
          "Call site parameters can't be annotated with @Argument for invoke dynamic "
          + "instructions, found at index '%s'",
          index);
    }
  },
  ADVICE_PARAMETER_RETURN_SHOULD_BE_LAST {
    ...
    public String apply(final Object[] objects) {
      final int index = (int) objects[0];
      return String.format(
          "Call site parameter annotated with @Return should be last, found at index '%s'",
          index);
    }
  },
  ADVICE_PARAMETER_RETURN_NON_AFTER_ADVICE {
    ...
    public String apply(final Object[] objects) {
      final int index = (int) objects[0];
      return String.format(
          "Call site parameter annotated with @Return should only be used on @After call "
          + "sites, found at index '%s'",
          index);
    }
  },
  ...
  ADVICE_AFTER_SHOULD_HAVE_THIS {
    @Override
    public String apply(final Object[] objects) {
      return "After advice first parameter should be annotated with @This or "
          + "@AllArguments(includeThis = true)";
    }
  },
  ...
  POINTCUT_SIGNATURE_INVALID {
    ...
    public String apply(final Object[] objects) {
      ...
      return String.format(
          "Pointcut signature '%s' does not match the regular expression '%s'",
          signature,
          pattern);
    }
  },
