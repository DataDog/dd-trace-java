...
- **Local agent with a skill harness (pilot):** run [dd-apm-sdk-review](.agents/skills/dd-apm-sdk-review/SKILL.md) on demand when asked. It is not required before every push. If any `P0` finding is reported, fix it or get explicit authorization and record the unresolved finding in the PR description (location and class of issue only — never paste secret values, tokens, credentials, or exploit details). `P1`/`P2` findings can be dismissed by the human. (Performance review, previously the standalone `/perf-review` skill, is now one of this skill's perspectives — see `.agents/dd-apm-sdk-review-overrides/reviewers/performance.md`.)
