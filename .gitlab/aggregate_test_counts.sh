...
#
# aggregate_test_counts.sh - Aggregate test counts from GitLab CI test jobs
#
# This script collects and aggregates test execution data from multiple test jobs
# in a GitLab CI pipeline. It processes test_counts_*.json files generated by
# individual test jobs and produces comprehensive summaries and reports.
#
# Workflow:
#   1. Finds all test_counts_*.json files from test jobs
...
#
# Usage:
#   aggregate_test_counts.sh [-v] [aggregate_dir]
#
# Options:
#   -v              Enable verbose output for debugging
#   aggregate_dir   Directory containing test_counts_*.json files
#                   (default: ./test_counts_aggregate)
#
# Outputs:
...
#   CI_PROJECT_URL      - GitLab project URL (for artifact links)
#
# See: docs/test-coverage.md for more details on the test counting system
#
...
aggregate_test_data() {
    ...
    local jq_program
    jq_program=$(cat <<-'JQ'
	# Reusable function to generate JVM version sort keys
	# Returns: [numeric_value, is_prefixed, version_string]
	# This ensures: 8 < 11 < 17 < 21 < graalvm17 < semeru21 < stable
	def jvm_sort_key:
	  [
	    # First: extract numeric value (8, 11, 17, 21, 25, etc.)
	    (if . == "stable" then 1000
	     elif ((. | gsub("[^0-9]"; "")) as $nums | $nums == "") then 0
	     else (. | gsub("[^0-9]"; "") | tonumber)
	     end),
	    # Second: plain numbers before prefixed versions (8 before semeru8, 17 before graalvm17)
	    (if (. | test("^[0-9]+$")) then 0 else 1 end),
	    # Third: alphabetically by full version string
	    .
	  ];
	...
	]) |
	{
	  ...
	  # Overall summary across all jobs
	  summary: {
	  ...
	  # Aggregation by JVM version (e.g., 8, 11, 17, graalvm17, stable)
	  by_jvm: (group_by(.jvm_version) | map({
	    ...
	    total_skipped: (map(.skipped_tests) | add),
	    job_count: length
	  }) | sort_by(.jvm_version | jvm_sort_key)),

	  # Aggregation by job kind and JVM (ignoring test splits)
	  # This groups all splits of the same job+JVM together
	  by_job_kind_and_jvm: (
	    ...
	    map({
	      ...
	      total_skipped: (map(.skipped_tests) | add),
	      split_count: length
	    }) | sort_by([.job_kind, (.jvm_version | jvm_sort_key)])
	  ),

	  # Pre-formatted table rows for markdown report (detailed breakdown)
	  table_rows: map(
	    [.test_category, .jvm_version, .ci_job_name, .total_tests, .passed_tests, .failed_tests, .skipped_tests] |
	    "| \(.[0]) | \(.[1]) | \(.[2]) | \(.[3]) | \(.[4]) | \(.[5]) | \(.[6]) |"
	  ),

	  # Detection of jobs with zero tests (potential issues)
	  zero_test_jobs: map(
