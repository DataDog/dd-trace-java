...
class JsonPathParserTest extends Specification {
  ...
  def "parse correct json-path patterns"() {
    ...
    path                            | expected
    '$.a'                           | p().key("a").build()
    '$.BarFoo'                      | p().key("BarFoo").build()
    '$.*'                           | p().any().build()
    '$[*]'                          | p().any().build()
    '$[* ]'                         | p().any().build()
    '$[ *]'                         | p().any().build()
    '$..b'                          | p().search().key("b").build()
    '$..*'                          | p().search().any().build()
    '$..fooBar'                     | p().search().key("fooBar").build()
    '$..[*]'                        | p().search().any().build()
    '$..[0]'                        | p().search().index(0).build()
    '$.phoneNumbers.*.number'       | p().key("phoneNumbers").any().key("number").build()
    '$.phoneNumbers[*].number'      | p().key("phoneNumbers").any().key("number").build()
    '$..number'                     | p().search().key("number").build()
    '$.foo..bar..number'            | p().key("foo").search().key("bar").search().key("number").build()
    '$["foo"]..["bar"]..["number"]' | p().key("foo").search().key("bar").search().key("number").build()
    '$.phoneNumbers[3].number'      | p().key("phoneNumbers").index(3).key("number").build()
    '$.phoneNumbers["3"].number'    | p().key("phoneNumbers").key("3").key("number").build()
    '$.phoneNumbers[ "3" ].number'  | p().key("phoneNumbers").key("3").key("number").build()
    '$.*.*'                         | p().any().any().build()
    '$[" a"]'                       | p().key(" a").build()
    '$["a "]'                       | p().key("a ").build()
