...
class OtlpGrpcRequestBodyTest {
  ...
  void writeToProducesGrpcFrameWithUncompressedFlagWhenNotGzipped() throws IOException {
    ...
    OtlpGrpcRequestBody body = new OtlpGrpcRequestBody(
        new OtlpPayload(ByteBuffer.wrap(data), "application/grpc+proto"),
        false);
    Buffer sink = new Buffer();

    body.writeTo(sink);
    // uncompressed flag
    assertEquals(0, sink.readByte());
    // 4-byte big-endian length
    assertEquals(data.length, sink.readInt());
    // payload
  ...
  void writeToProducesGrpcFrameWithCompressedFlagAndGzipDataWhenGzipped() throws IOException {
    ...
    // 4-byte big-endian gzip content length
    int gzipLength = sink.readInt();
