  ...
  private static final class MetaWriter implements MetadataConsumer {
    ...
    public void accept(Metadata metadata) {
      ...
      for (Map.Entry<String, Object> tag : metadata.getTags().entrySet()) {
        ...
        } else if (key.startsWith(LLMObsTags.LLMOBS_METRIC_PREFIX)
            && tag.getValue() instanceof Number) {
          ++metricsSize;
      ...
      for (Map.Entry<String, Object> tag : metadata.getTags().entrySet()) {
        if (tag.getKey().startsWith(LLMObsTags.LLMOBS_METRIC_PREFIX)
            && tag.getValue() instanceof Number) {
          writable.writeString(
              tag.getKey().replaceFirst(LLMObsTags.LLMOBS_METRIC_PREFIX, ""), null);
