...
public class LLMObsSpanMapper implements RemoteMapper {
  ...
  private static final String INTERNAL_TAG_SPAN_NAME = "span_operation_name";
  ...
  private static final String PARENT_ID_TAG_INTERNAL_FULL = LLMObsTags.LLMOBS_TAG_PREFIX + "parent_id";
  ...
  public void map(List<? extends CoreSpan<?>> trace, Writable writable) {
    List<? extends CoreSpan<?>> llmobsSpans = trace.stream().filter(LLMObsSpanMapper::isLLMObsSpan).collect(Collectors.toList());
  ...
  private static final class MetaWriter implements MetadataConsumer {
    ...
    private static final Set<String> TAGS_FOR_REMAPPING = Collections.unmodifiableSet(new HashSet<>(Arrays.asList(
        LLMObsTags.INPUT,
        LLMObsTags.OUTPUT,
        LLMObsTags.MODEL_NAME,
        LLMObsTags.MODEL_PROVIDER,
        LLMObsTags.MODEL_VERSION,
        LLMObsTags.METADATA
    )));
    ...
    public void accept(Metadata metadata) {
      ...
      for (Map.Entry<String, Object> tag : metadata.getTags().entrySet()) {
        ...
        if (key.equals(Tags.SPAN_KIND)) {
          spanKind =String.valueOf(tag.getValue());
        } else if (TAGS_FOR_REMAPPING.contains(key)) {
          tagsToRemapToMeta.put(key, tag.getValue());
        } else if (key.startsWith(LLMObsTags.LLMOBS_METRIC_PREFIX) && tag.getValue() instanceof Number) {
          ++metricsSize;
        } else if (key.startsWith(LLMObsTags.LLMOBS_TAG_PREFIX)) {
          if (TAGS_FOR_REMAPPING.contains(key.replaceFirst(LLMObsTags.LLMOBS_TAG_PREFIX, ""))) {
            tagsToRemapToMeta.put(key, tag.getValue());
          } else {
            ++tagsSize;
          }
      ...
      for (Map.Entry<String, Object> tag : metadata.getTags().entrySet()) {
        if (tag.getKey().startsWith(LLMObsTags.LLMOBS_METRIC_PREFIX) && tag.getValue() instanceof Number) {
          writable.writeString(tag.getKey().replaceFirst(LLMObsTags.LLMOBS_METRIC_PREFIX, ""), null);
      ...
      for (Map.Entry<String, Object> tag : metadata.getTags().entrySet()) {
        ...
        if (!tagsToRemapToMeta.containsKey(tag.getKey()) && tag.getKey().startsWith(LLMObsTags.LLMOBS_TAG_PREFIX)) {
          String key = tag.getKey().replaceFirst(LLMObsTags.LLMOBS_TAG_PREFIX, "");
          String toWrite = key + ":" + value;
          writable.writeObject(toWrite, null);
//          if (value instanceof Number || value instanceof Boolean || value instanceof String) {
//            writable.writeString(key + ":" + value, null);
//          }
//          String val = convertToJson(value);
//          writable.writeString(key + ":" + val, null);
      ...
      for (Map.Entry<String, Object> tag : tagsToRemapToMeta.entrySet()) {
        ...
        if (key.equals(LLMObsTags.INPUT) || key.equals(LLMObsTags.OUTPUT) ) {
...
//            val = convertToJson(val);
    ...
    private static boolean JSONSerializable(Object o) {
      return o instanceof String || o instanceof String[] || o instanceof Iterable || o instanceof Map || o instanceof Number || o instanceof Boolean;
    }
  ...
  private static class PayloadV2 extends Payload {
    ...
    public RequestBody toRequest() {
      ...
      // working json
//      String jsonStr = "{\"_dd.stage\": \"raw\", \"_dd.tracer_version\": \"java\", \"event_type\": \"span\", \"spans\": [{\"trace_id\": \"deadbeef\", \"span_id\": \"202502061557\", \"parent_id\": \"undefined\", \"name\": \"agent_root\", \"start_ns\": 1738867324412779000, \"duration\": 225467000, \"status\": \"ok\", \"meta\": {\"span.kind\": \"agent\", \"input\": {\"value\": \"agent root input0.8541088675419499\"}, \"output\": {\"value\": \"output\"}, \"metadata\": {}}, \"metrics\": {}, \"tags\": [\"version:\", \"env:staging\", \"service:test-gary-java\", \"source:integration\", \"ml_app:test-gary-java\", \"language:java\", \"error:0\"]}]}";
//      RequestBody reqBody = jsonRequestBodyOf(jsonStr.getBytes());
  ...
  private String readMsgPackToStr(ByteBuffer body) {
    ...
    String responseJson = "";
    try {
      unpacker = new MessagePack.UnpackerConfig()
