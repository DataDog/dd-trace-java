...
public class InferredProxyCodec {
  //
  //  private static final Logger log = LoggerFactory.getLogger(InferredProxyCodec.class);
  ...
  //     * @return {@code null} for failed context extraction, a {@link TagContext} instance for
  // partial
  ...
  //    EnumMap<TracePropagationStyle, Injector> result = new
  // EnumMap<>(TracePropagationStyle.class);
  ...
  //    /** Cached context key-values (even indexes are header names, odd indexes are header
  // values). */
  //    private final List<String> keysAndValues;
  //    /**
  //     * The parent span identifier from {@link DatadogHttpCodec#SPAN_ID_KEY} header formatted as
  // 16
  ...
  //
  //    @Override
  //    public void forEachKey(ExtractionCache<?> carrier, AgentPropagation.KeyClassifier
  // classifier) {
  ...
  //
  //  /**
  //   * Checks if trace identifier matches, even if they are not encoded using the same size
  // (64-bit vs
  //   * 128-bit).
  //   *
