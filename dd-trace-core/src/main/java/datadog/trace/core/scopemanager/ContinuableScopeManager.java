...
public class ContinuableScopeManager extends ScopeInterceptor.DelegatingInterceptor
  ...
  private Scope handleSpan(
    ...
    final Scope scope =
        delegate instanceof ContinuableScopeManager
            ? ((ContinuableScopeManager) delegate)
                .handleSpan(active, null, span, ScopeSource.INSTRUMENTATION)
            : delegate.handleSpan(span);
    final ContinuableScope continuableScope =
        new ContinuableScope(active, continuation, scope, source);
