...
final class CoreHandlers {
  ...
  CoreHandlers() {
    ...
    this.resource = new PropertyCardinalityHandler(
        "resource",
    ...
    this.httpEndpoint = new PropertyCardinalityHandler(
        "http_endpoint",
        config.getTraceStatsCardinalityLimit("http_endpoint", MetricCardinalityLimits.HTTP_ENDPOINT),
        MetricCardinalityLimits.USE_BLOCKED_SENTINEL);
    // Fixed limits — hardcoded, not user-configurable.
    this.service = new PropertyCardinalityHandler(
        "service",
    ...
    this.operation = new PropertyCardinalityHandler(
        "operation",
        "name",
        MetricCardinalityLimits.OPERATION,
        MetricCardinalityLimits.USE_BLOCKED_SENTINEL);
    this.serviceSource = new PropertyCardinalityHandler(
        "service_source",
        MetricCardinalityLimits.SERVICE_SOURCE,
        MetricCardinalityLimits.USE_BLOCKED_SENTINEL);
    this.type = new PropertyCardinalityHandler(
        "type",
        MetricCardinalityLimits.TYPE,
        MetricCardinalityLimits.USE_BLOCKED_SENTINEL);
    this.spanKind = new PropertyCardinalityHandler(
        "span_kind",
        MetricCardinalityLimits.SPAN_KIND,
        MetricCardinalityLimits.USE_BLOCKED_SENTINEL);
    this.httpMethod = new PropertyCardinalityHandler(
        "http_method",
        MetricCardinalityLimits.HTTP_METHOD,
        MetricCardinalityLimits.USE_BLOCKED_SENTINEL);
    this.grpcStatusCode = new PropertyCardinalityHandler(
        "grpc_status_code",
        MetricCardinalityLimits.GRPC_STATUS_CODE,
        MetricCardinalityLimits.USE_BLOCKED_SENTINEL);
    this.handlers = new PropertyCardinalityHandler[] {
        resource,
        service,
        operation,
        serviceSource,
