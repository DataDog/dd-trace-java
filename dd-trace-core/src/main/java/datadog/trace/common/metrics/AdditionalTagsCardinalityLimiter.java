 ...
 *
 * <p>Single global counter, single-threaded -- the aggregator thread is the sole writer, so a
 * plain {@code int} is sufficient (no {@code AtomicInteger}). The counter goes up by one each
 * time we admit a brand-new {@link AggregateEntry} that includes any non-null additional tag
 * values. When the counter reaches the cap, any further new entries have all their additional
 * tag values replaced with the per-key {@code "<key>:blocked_by_tracer"} sentinel before they
 * become part of the hash + match keys -- so the blocked spans collapse into a small number of
 * "shape" entries rather than into the no-additional-tags base bucket. Spans whose full canonical
 * already exists in the table merge into it regardless of the cap.
 *
 * <p>The counter and both one-shot warn flags reset every flush via {@link #resetBucket()}.
 *
 * <p>Acknowledged spec deviation: the CSS v1.3.0 spec requires per-tag isolation. This is a
 * single global counter for simplicity. A misconfigured tag can starve another tag's admission of
 * new entries within a bucket, but every span still gets emitted with its dimension keys
 * preserved (values masked) and its base stats unchanged.
 */
final class AdditionalTagsCardinalityLimiter {

  private static final Logger log =
      LoggerFactory.getLogger(AdditionalTagsCardinalityLimiter.class);
   ...
   * AdditionalTagsSchema#MAX_ADDITIONAL_TAG_VALUE_LENGTH} -- caller should substitute the
   * schema's per-key blocked sentinel and fire the health metric. We return {@code null} rather
   * than the {@link AdditionalTagsSchema#BLOCKED_VALUE} string here so the caller can plug in the
   * pre-built {@code UTF8BytesString} sentinel directly without re-canonicalizing.
   */
