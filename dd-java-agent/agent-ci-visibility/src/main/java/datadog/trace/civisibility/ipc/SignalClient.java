...
public class SignalClient implements AutoCloseable {
  ...
  public SignalResponse send(Signal signal) throws IOException {
    ...
    LOGGER.debug(
        "Sending signal of type {} and size {} bytes",
        signal.getType(),
        message.remaining());

    ByteBuffer header = ByteBuffer.allocate(Integer.BYTES + 1);
    // +1 is for signal type
    header.putInt(message.remaining() + 1);
    ...
    public SignalClient create() {
      ...
      try {
        return new SignalClient(
            signalServerAddress,
            config.getCiVisibilitySignalClientTimeoutMillis());
      } catch (IOException e) {
        throw new RuntimeException(
            "Could not instantiate signal client. Address: " + signalServerAddress,
            e);
      }
    }
  }
