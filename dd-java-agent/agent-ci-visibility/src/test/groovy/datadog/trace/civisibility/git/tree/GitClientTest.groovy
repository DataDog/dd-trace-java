...
class GitClientTest extends Specification {
  ...
  def "test get remote url"() {
    ...
    when:
    def gitClient = new GitClient(tempDir.toString(), GIT_COMMAND_TIMEOUT_MILLIS)
  ...
  def "test get latest commits"() {
    ...
    when:
    def gitClient = new GitClient(tempDir.toString(), GIT_COMMAND_TIMEOUT_MILLIS)
    def commits = gitClient.getCommits(10, "25 years ago")
  ...
  def "test get objects"() {
    ...
    when:
    def gitClient = new GitClient(tempDir.toString(), GIT_COMMAND_TIMEOUT_MILLIS)
    def objects = gitClient.getObjects("HEAD", "25 years ago", ["98cd7c8e9cf71e02dc28bd9b13928bee0f85b74c"])
