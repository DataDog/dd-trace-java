...
public class DDLLMObsSpan implements LLMObsSpan {
  ...
  public DDLLMObsSpan(
    ...
    AgentTracer.SpanBuilder spanBuilder =
        AgentTracer.get()
            // set operation name as span kind so that apm span will have span kind as the operation name, we will remap this later at write time
    ...
    this.span.setTag(LLMObsTags.LLMOBS_TAG_PREFIX + PARENT_ID_TAG_INTERNAL, activeSpanCtxInfo.getParentSpanID());
  ...
  public void setMetrics(Map<String, Number> metrics) {
    ...
    for (Map.Entry<String, Number> entry : metrics.entrySet()) {
      this.span.setMetric(LLMObsTags.LLMOBS_METRIC_PREFIX + entry.getKey(), entry.getValue().doubleValue());
    }
  }
