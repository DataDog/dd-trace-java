...
public final class ShouldInjectFieldsMatcher
  ...
  protected boolean doMatch(TypeDescription typeDescription) {
    if (!subclassOfKey.matches(typeDescription)) {
      // ignore types that are completely unrelated to the key
      return false;
    }
    ...
    if (skipType != null && ExcludeFilter.exclude(skipType, matchedType)) {
      ...
      if (log.isDebugEnabled()) {
        log.debug(
            "Skipping context-store field - instrumentation.target.class={} instrumentation."
            + "target.context={}->{}",
            matchedType,
            keyType,
            valueType);
    ...
    if (log.isDebugEnabled()) {
      if (!shouldInject && null != injectionTarget) {
        log.debug(
            "Will not add context-store field, alternate target found {} - instrumentation."
            + "target.class={} instrumentation.target.context={}->{}",
            injectionTarget,
            matchedType,
            keyType,
            valueType);
