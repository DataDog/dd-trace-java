...
class WAFModuleSpecification extends DDSpecification {
  ...
  ConfigurationMapListener listener
  ...
  void setup() {
    ...
    () -> {}, wafBuilder)
    service.init()
    listener = service.asmDDListener
  }
  ...
  private void send(String configKey, Object map){
    listener.accept(configKey, map as Map<String, Object>, null)
  }
  ...
  private void initialRuleAdd(String location = "test_multi_config.json") {
    ...
    listener.accept("initial_waf", ADAPTER.fromJson(Okio.buffer(Okio.source(stream))), null)
    readyToHandle()
  }
  ...
  void initialRuleAddWithMap(Map<String, Object> definition) {
    wafBuilder?.close()
    setup()
    listener.accept("waf", definition, null)
    readyToHandle()
  }
  ...
  void 'override on_match through reconfiguration'() {
    ...
    1 * ctx.getOrCreateWafContext(_ as WafHandle, false)
    2 * tracer.activeSpan()
  ...
  void 'provide data through the initial config'() {
    ...
    wafBuilder.removeConfig('initial_waf')
