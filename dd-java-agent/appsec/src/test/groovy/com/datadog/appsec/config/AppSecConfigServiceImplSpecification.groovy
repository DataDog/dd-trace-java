...
class AppSecConfigServiceImplSpecification extends DDSpecification {
  ...
  void setUpTest() {
    ...
    appSecConfigService = new AppSecConfigServiceImpl(config, poller, reconf, wafBuilder)
  }
  ...
  static class SavedListeners {
    WafBuilder wafBuilder
    SavedListeners(WafBuilder wafBuilder) {
      this.wafBuilder = wafBuilder
    }
    ConfigurationMapListener savedWafDataChangesListener = (configKey, rawConfig, hinter) -> {
      if (rawConfig == null) {
  ...
  void 'activation without custom config provides valid configuration'() {
    ...
    setUpTest()
    SavedListeners listeners = new SavedListeners(wafBuilder)
  ...
  void 'provides updated configuration to waf subscription'() {
    ...
    'ignored config key',
    [rules: [[id: "foo", name: "foo", tags:
          [type: "php_code_injection", crs_id: "933140", category: "attack_attempt", cwe: "94",
            capec: "1000/225/122/17/650", confidence: "1", module: "waf"],
          conditions:
          [[operator: "ip_match", parameters:
              [data: "suspicious_ips_data_id", inputs: [[address: "http.client_ip"]]]]]
          , type: "", data: []]]]
    , null)
    listeners.savedWafDataChangesListener.accept(
    'ignored config key',
    [rules_override: [[rules_target: [[rule_id: "foo"]], enabled: false]]], null)
    listeners.savedConfEndListener.onConfigurationEnd()
  ...
  void 'configuration pull out'() {
    ...
    when:
    listeners.savedWafDataChangesListener.accept('asm_dd config', null, null)
    listeners.savedWafDataChangesListener.accept('asm conf', null, null)
    listeners.savedConfEndListener.onConfigurationEnd()
  ...
  void 'update auto user instrum mode via remote-config'() {
    given:
    def listeners = new SavedListeners(wafBuilder)
  ...
  private static AppSecFeatures autoUserInstrum(String mode) {
    return new AppSecFeatures().tap {
      features ->
      features.autoUserInstrum = new AppSecFeatures.AutoUserInstrum().tap {
        instrum ->
        instrum.mode = mode
      }
    }
