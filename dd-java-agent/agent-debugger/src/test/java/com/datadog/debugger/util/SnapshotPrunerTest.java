...
class SnapshotPrunerTest {
  ...
  public void noPruning() {
    ...
    assertEquals(
        "{\"foo\":[[],[],[]]}",
        SnapshotPruner.prune("{\"foo\":[[],[],[]]}", MAX_SNAPSHOT_SIZE, 0)
    );
    assertEquals(
        "{\"foo\":\"bar\"}",
        SnapshotPruner.prune("{\"foo\":\"bar\"}", MAX_SNAPSHOT_SIZE, 0)
    );
  ...
  public void sliceSmallSnapshot() throws Exception {
    ...
    String smallSnapshot = utils.TestHelper
      .getFixtureContent("/com/datadog/debugger/util/smallSnapshot_pruned0.json")
      .trim();
    assertEquals(smallSnapshot, SnapshotPruner.prune(inputSmallSnapshot, 1500, MIN_LEVEL));
    smallSnapshot = utils.TestHelper
      .getFixtureContent("/com/datadog/debugger/util/smallSnapshot_pruned1.json")
      .trim();
    assertEquals(smallSnapshot, SnapshotPruner.prune(inputSmallSnapshot, 1250, MIN_LEVEL));
    smallSnapshot = utils.TestHelper
      .getFixtureContent("/com/datadog/debugger/util/smallSnapshot_pruned2.json")
      .trim();
    assertEquals(smallSnapshot, SnapshotPruner.prune(inputSmallSnapshot, 1100, MIN_LEVEL));
    smallSnapshot = utils.TestHelper
      .getFixtureContent("/com/datadog/debugger/util/smallSnapshot_pruned3.json")
      .trim();
    assertEquals(smallSnapshot, SnapshotPruner.prune(inputSmallSnapshot, 1000, MIN_LEVEL));
    smallSnapshot = utils.TestHelper
      .getFixtureContent("/com/datadog/debugger/util/smallSnapshot_pruned4.json")
      .trim();
