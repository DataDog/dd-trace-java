...
public class OverheadContext {
  ...
  @Nullable
  private final Map<String, int[]> copyMap;
  ...
  @Nullable
  private final Map<String, AtomicIntegerArray> requestMap;
      ...
      for (int i = 0; i < numberOfVulnerabilities; i++) {
        ...
        if (counter > 0) {
          AtomicIntegerArray globalCountMap =
              globalMap.computeIfAbsent(endpoint, value -> new AtomicIntegerArray(
