...
class IastSecurityControlTransformerForkedTest extends DDSpecification{
  ...
  def setupSpec() {
    final config = "${SANITIZER};${SANITIZE_VALIDATE_OBJECT};${SANITIZE_INPUTS};${SANITIZE_MANY_INPUTS};${INPUT_VALIDATOR_VALIDATE_ALL};${INPUT_VALIDATOR_VALIDATE_OVERLOADED};${INPUT_VALIDATOR_VALIDATE_RETURNING_INT};${INPUT_VALIDATOR_VALIDATE_OBJECT}"
  ...
  void 'test sanitize'(){
    ...
    then:
    1 * iastModule.markIfTainted( 'Sanitized', marks)
    0 * _
    ...
    'sanitize' | ['test']
    'sanitizeObject' | ['test']
    'sanitizeInputs' | ['test1', new Object(), 27i]
    'sanitizeManyInputs' | ['test', 'test2', 'test3', 'test4', 'test5', 'test6', 'test7', 'test8', 'test9', 'test10']
  }
