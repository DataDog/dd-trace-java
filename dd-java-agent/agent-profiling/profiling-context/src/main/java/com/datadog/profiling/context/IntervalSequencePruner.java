...
final class IntervalSequencePruner {
  ...
  LongIterator pruneIntervals(LongSequence sequence, long timestamp) {
    int lastTransition = TRANSITION_NONE;
    ...
    while (iterator.hasNext()) {
      ...
      if (transition == TRANSITION_STARTED) {
        if (lastTransition == TRANSITION_STARTED) {
        ...
        } else if (lastTransition > TRANSITION_STARTED) {
          ...
          if (lastTransition == TRANSITION_MAYBE_FINISHED) {
      ...
      } else if (transition == TRANSITION_MAYBE_FINISHED) {
        if (lastTransition == TRANSITION_NONE) {
        ...
        } else if (lastTransition == TRANSITION_FINISHED) {
          // 'finish' followed by 'maybe finished' turns into 'finished'
          transition = lastTransition;
        }
      } else if (transition == TRANSITION_FINISHED) {
        if (lastTransition == TRANSITION_NONE) {
          // dangling transition - remove it
