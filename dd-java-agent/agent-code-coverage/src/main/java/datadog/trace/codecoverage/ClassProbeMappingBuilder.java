...
/**
 * Builds a {@link ClassProbeMapping} by parsing the class bytecode using JaCoCo's {@link
 ...
 *
 * <p>Uses two passes over the bytecode: one via {@link ClassProbesAdapter} to build probe-to-line
 * mappings (single-pass, O(1) regardless of probe count), and one via JaCoCo's {@link Analyzer} to
 * obtain the filtered set of executable lines (applying FinallyFilter, etc.). This is significantly
 * faster than the N+1 pass approach while remaining correct.
 */
final class ClassProbeMappingBuilder {
  ...
  static ClassProbeMapping build(
    ...
    // One Analyzer pass: get filtered executable lines (handles FinallyFilter, etc.)
    BitSet executableLines = filteredExecutableLines(classBytes);
    if (executableLines == null) {
      executableLines = visitor.executableLines;
    }
    return visitor.toMapping(classId, className, probeCount, executableLines);
  }

  /**
   * Runs JaCoCo's Analyzer (with no probes) to get the set of executable lines after all filters
   * (FinallyFilter, bridge methods, etc.) have been applied. Returns null on failure.
   */
  private static BitSet filteredExecutableLines(byte[] classBytes) {
    try {
  ...
  private static final class ProbeMappingVisitor extends ClassProbesVisitor {
    ...
    ClassProbeMapping toMapping(
        long classId, String className, int probeCount, BitSet filteredExecLines) {
      int[][] probeToLinesArray = new int[probeCount][];
      for (int p = 0; p < probeCount; p++) {
        BitSet lines = probeToLines.get(p);
        if (lines != null) {
          // Intersect with filtered executable lines so probe lines never include
          // lines that JaCoCo's filters removed (e.g. duplicated finally blocks).
          BitSet filtered = (BitSet) lines.clone();
          filtered.and(filteredExecLines);
          probeToLinesArray[p] = bitSetToArray(filtered);
        } else {
          probeToLinesArray[p] = new int[0];
        }
      }
      return new ClassProbeMapping(
          classId, className, sourceFile, filteredExecLines, probeToLinesArray);
    }
   ...
   * records probe-to-instruction associations. After all instructions are replayed, jump targets
   * are wired and predecessor chains are walked to collect covered lines per probe.
   *
  ...
  private static final class MethodMapper extends MethodProbesVisitor {
    ...
    public void visitProbe(int probeId) {
      addProbe(probeId);
      currentInsn = null; // noSuccessor: only standalone probes break the chain
    }
