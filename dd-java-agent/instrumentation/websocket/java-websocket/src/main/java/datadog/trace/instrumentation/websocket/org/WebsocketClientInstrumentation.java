...
public class WebsocketClientInstrumentation
  ...
  public void methodAdvice(MethodTransformer transformer) {
    transformer.applyAdvice(
        isMethod().and(isPublic()).and(nameStartsWith("onWebsocketOpen")).and(takesArguments(2)),
        getClass().getName() + "$OnOpenAdvice");

    transformer.applyAdvice(
        isConstructor().and(takesArgument(1, named("org.java_websocket.drafts.Draft"))),
        getClass().getName() + "$ClientConstructorAdvice");

    transformer.applyAdvice(
        isMethod().and(isPublic()).and(named("connect")).and(takesArguments(0)),
        getClass().getName() + "$ConnectAdvice");

    transformer.applyAdvice(
        isMethod().and(isPublic()).and(nameStartsWith("onWebsocketMessage")).and(takesArguments(2)),
        getClass().getName() + "$OnMessageAdvice");

    transformer.applyAdvice(
        isMethod().and(isPublic()).and(nameStartsWith("onWebsocketClose")).and(takesArguments(4)),
        getClass().getName() + "$OnCloseAdvice");

    transformer.applyAdvice(
        isMethod().and(isPublic()).and(named("onWebsocketError")).and(takesArguments(2)),
        getClass().getName() + "$ErrorAdvice");
  }

  public static class ConnectAdvice {
    @Advice.OnMethodEnter(suppress = Throwable.class)
    public static AgentScope onEnter(@Advice.This WebSocketClient client) {
    ...
    public static void onEnter(@Advice.Argument(value = 1, readOnly = false) Draft protocolDraft) {
      ...
      if (protocolDraft instanceof Draft_6455 && !(protocolDraft instanceof TraceDraft_6455)) {
        try {
          ...
          // Keep the original draft if wrapping fails.
        }
      }
    }
  }
  ...
  public static class OnOpenAdvice {
    ...
    public static AgentScope onEnter(
        ...
        @Advice.This WebSocketClient client) {
      AgentSpan span =
          InstrumentationContext.get(WebSocketClient.class, AgentSpan.class).get(client);
      if (span == null) {
        return noopScope();
      }
      ...
      InstrumentationContext.get(WebSocketClient.class, AgentSpan.class).remove(client);
      WebsocketAgentSpanContext spanContext =
          new WebsocketAgentSpanContext(span.spanContext(), span.getTag(Tags.SPAN_KIND));
      InstrumentationContext.get(WebSocket.class, WebsocketAgentSpanContext.class)
          .put(conn, spanContext);
  ...
  public static class OnMessageAdvice {
    @Advice.OnMethodEnter(suppress = Throwable.class)
    public static AgentScope onEnter(
        @Advice.Argument(value = 0) WebSocket engine, @Advice.Argument(value = 1) Object message) {
      WebsocketAgentSpanContext context =
          InstrumentationContext.get(WebSocket.class, WebsocketAgentSpanContext.class).get(engine);
      if (context == null) {
        return activateSpan(noopSpan());
      }
      final AgentSpan wsSpan = CLIENT_DECORATE.onMessage(message, context.getSpanContext());
  ...
  public static class OnCloseAdvice {
    ...
    public static AgentScope onEnter(
      ...
      WebsocketAgentSpanContext context =
          InstrumentationContext.get(WebSocket.class, WebsocketAgentSpanContext.class).get(conn);
      if (context == null) {
        AgentSpan agentSpan =
            InstrumentationContext.get(WebSocketClient.class, AgentSpan.class).get(client);
        if (agentSpan != null) {
