...
public class SessionInstrumentation
  ...
  public void methodAdvice(MethodTransformer transformer) {
    ...
    transformer.applyAdvice(
        isPublic()
            .and(
                named("addMessageHandler")
                    .and(takesArgument(1, named(namespace + ".websocket.MessageHandler")))),
    ...
    transformer.applyAdvice(
        isPublic().and(named("close")).and(takesNoArguments()),
    ...
    public static void onExit(
      ...
      if (handler != null) {
        final AgentSpan handshakeSpan =
            InstrumentationContext.get(Session.class, AgentSpan.class).get(session);
        if (handshakeSpan != null) {
          ...
          InstrumentationContext.get(MessageHandler.class, HandlerContext.Receiver.class)
              .put(handler, new HandlerContext.Receiver(handshakeSpan, session.getId()));
        }
      }
    }
  }
  ...
  public static class SessionCloseAdvice {
    ...
    public static AgentScope before(
      ...
      handlerContext = new HandlerContext.Sender(handshakeSpan, session.getId());
      return activateSpan(
