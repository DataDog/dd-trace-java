...
import datadog.trace.instrumentation.vertx_3_4.server.RoutingContextJsonAdvice
...
<root>&xxe;</root>"""
  ...
  private Method getIsXmlContentMethod() {
    def method = RoutingContextJsonAdvice.getDeclaredMethod("isXmlContent", String)
    method.setAccessible(true)
    return method
  }

  private Method getParseXmlToWafFormatMethod() {
    def method = RoutingContextJsonAdvice.getDeclaredMethod("parseXmlToWafFormat", String)
    method.setAccessible(true)
    return method
  }

  private Method getProcessObjectForWafMethod() {
    def method = RoutingContextJsonAdvice.getDeclaredMethod("processObjectForWaf", Object)
    method.setAccessible(true)
    return method
  }
}
