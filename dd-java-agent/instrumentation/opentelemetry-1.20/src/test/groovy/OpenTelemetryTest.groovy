...
class OpenTelemetryTest extends AgentTestRunner {
  @Subject
  def tracer = GlobalOpenTelemetry.get().tracerProvider.get("test-inst")
//  def httpPropagator = OpenTelemetry.getPropagators().httpTextFormat
  ...
  void configurePreAgent() {
    ...
    // Required as instrumentation is not enabled by default
  ...
  def "test span tags"() {
    ...
    def tracerProvider = openTelemetry.getTracerProvider()
