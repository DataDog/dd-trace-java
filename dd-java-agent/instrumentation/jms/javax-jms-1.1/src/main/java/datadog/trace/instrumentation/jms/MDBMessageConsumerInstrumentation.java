  ...
  public static class MDBAdvice {
    ...
    public static AgentScope methodEnter(@Advice.Argument(0) final Message message) {
      ...
      try {
        ...
        destinationName = CONSUMER_DECORATE.getDestinationName(destination);
      ...
      CONSUMER_DECORATE.onConsume(span, message, consumerResourceName, destinationName, "process");

      if (Config.get().isDataStreamsEnabled()
          && null != destinationName
          && !destinationName.isEmpty()) {
        final String tech = messageTechnology(message);
        DataStreamsTags tags = create(tech, INBOUND, destinationName);
