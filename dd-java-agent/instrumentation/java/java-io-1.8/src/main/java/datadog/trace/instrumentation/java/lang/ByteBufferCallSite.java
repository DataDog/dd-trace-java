...
public class ByteBufferCallSite {
  ...
  public static ByteBuffer afterWrap(
      @CallSite.Argument @Nullable final byte[] bytes,
      @CallSite.Return final ByteBuffer result
  ) {
    ...
    try {
      // keep ranges
      module.taintObjectIfTainted(result, bytes, true, NOT_MARKED);
  ...
  public static byte[] afterArray(
      @CallSite.This @Nonnull final ByteBuffer buffer,
      @CallSite.Return final byte[] bytes
  ) {
    if (bytes == null || bytes.length == 0) {
      return bytes;
    }
    ...
    try {
      // keep ranges
      module.taintObjectIfTainted(bytes, buffer, true, NOT_MARKED);
    } catch (final Throwable e) {
