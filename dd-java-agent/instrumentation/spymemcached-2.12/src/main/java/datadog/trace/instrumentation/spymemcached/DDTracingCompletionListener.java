...
@Slf4j
public abstract class DDTracingCompletionListener<T> {
  ...
  static final String SPAN_TYPE = "spymemcached";
  ...
  public DDTracingCompletionListener(final Tracer tracer, String methodName) {
    this.tracer = tracer;
    scope = buildSpan(getOperationName(methodName));
  }

  private Scope buildSpan(String operation) {
  ...
  protected void closeSpan(T future) {
