...
import akka.http.scaladsl.model.HttpRequest as sHttpRequest
...
class AkkaIastTestWebServer extends AllDirectives implements Closeable {

  int port
  Closure stop

  static void main(String[] args) {
    def webServer = new AkkaIastTestWebServer()
    webServer.start()
    System.in.read()
    webServer.stop()
  }
  ...
  void start() {
    ActorSystem system = ActorSystem.create('akka-http-server')
    ...
    Route routes = pathPrefix('iast') {
      ...
      path('json_mufeu') {
        post {
          ...
          Jackson.unmarshaller(Map) as akka.http.scaladsl.unmarshalling.Unmarshaller) as Unmarshaller) {
            Map map ->
            def mmap = map.collectEntries {
              e ->
              [t(e.key), e.value instanceof List ? e.value.collect {
