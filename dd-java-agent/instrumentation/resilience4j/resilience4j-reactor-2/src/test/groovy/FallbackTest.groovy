...
class FallbackTest extends AgentTestRunner {
  ...
  def "retry with fallback"() {
    ...
    ConnectableFlux<String> connection = Flux
      ...
      .transformDeferred(ReactorOperatorFallbackDecorator.decorateRetry(RetryOperator.of(Retry.of("R0", RetryConfig.custom().maxAttempts(1).build())), Flux.just("Fallback").map({ it -> serviceCall(it) })))
  ...
  def "fallback"() {
    ...
    .transformDeferred(ReactorOperatorFallbackDecorator.decorateRetry(RetryOperator.of(retry), Flux.just("Fallback").map({ it -> serviceCall(it) })))
    .publishOn(Schedulers.boundedElastic())
    .publish()
