...
import datadog.trace.bootstrap.instrumentation.api.{AgentScope, AgentSpan, AgentTracer}
...
object ClientWrapper {
  ...
  private def wrapClientResource[F[_]: ConcurrentEffect](client: Client[F]): Client[F] =
    Client { req: Request[F] =>
      for {
        requestAndScopeAndDecorator <- Resource.eval(startScopeAndWrapRequest(req))
        response                    <- client.run(requestAndScopeAndDecorator._1)
        newResponse <- Resource.eval(
