...
class StringConcatFactoryCallSiteTest extends AgentTestRunner {
  ...
  def 'test string concat factory with multiple args'() {
    ...
    then:
    result == expected
    1 * iastModule.onStringConcatFactory(first, first.join(), '\u0001\u0001', [] as Object[], [0, 1] as int[])
    1 * iastModule.onStringConcatFactory(second, second.join(), '\u0001\u0001', [] as Object[], [0, 1] as int[])
    1 * iastModule.onStringConcatFactory(third, third.join(), '\u0001\u0001', [] as Object[], [0, 1] as int[])
    1 * iastModule.onStringConcatFactory(fourth, fourth.join(), '\u0001\u0001', [] as Object[], [0, 1] as int[])
    0 * _
  }
