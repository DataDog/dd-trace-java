...
import datadog.trace.instrumentation.springweb7.SetupSpecHelper
import datadog.trace.instrumentation.springweb7.SpringWebHttpServerDecorator
...
class SpringBootBasedTest extends HttpServerTest<ConfigurableApplicationContext> {
  ...
  Map<String, Serializable> expectedExtraErrorInformation(ServerEndpoint endpoint) {
    // Spring Framework 7 DispatcherServlet always throws NoHandlerFoundException for 404s
  ...
  void assertEndpointDiscovery(final List<?> endpoints) {
    ...
    discovered.values().each {
      ...
      assert it.metadata['handler'] == 'datadog.trace.instrumentation.springweb7.boot.TestController#discovery()'
    }
  }
  ...
  void 'tainting on matrix var'() {
    ...
    then:
    // spring-security filter causes uri matching to happen twice (or three times in recent spring versions)
    (1.._) * mod.taintString(_ as IastContext, 'a=x,y', SourceTypes.REQUEST_PATH_PARAMETER, 'var')
