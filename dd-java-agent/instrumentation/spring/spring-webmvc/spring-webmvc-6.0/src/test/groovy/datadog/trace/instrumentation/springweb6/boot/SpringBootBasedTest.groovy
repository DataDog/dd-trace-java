...
class SpringBootBasedTest extends HttpServerTest<ConfigurableApplicationContext> {
  ...
  class SpringBootServer implements WebsocketServer {
    ...
    void start() {
      ...
      context = app.run()
      // Resolved dynamically (Groovy) so this compiles against both Spring Boot 3 and 4, which
      // relocated ServletWebServerApplicationContext to a different package.
