...
class SpringBootBasedTest extends AppSecHttpServerTest<ConfigurableApplicationContext> {
  ...
  class SpringBootServer implements HttpServer {
    ...
    void start() {
      ...
      context = app.run()
      // Resolved dynamically (Groovy) — ServletWebServerApplicationContext relocated in Boot 4
  ...
  def "test failed login with non existing user"() {
    ...
    // Spring Security 7 no longer exposes whether the user existed on a failed login,
    // so the tag is absent. See APPSEC-12765.
