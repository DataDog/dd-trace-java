...
public class AwsNameCache {
  ...
  private static final QualifiedClassNameCache CLASS_NAME_CACHE = new QualifiedClassNameCache(
      ...
      Functions.SuffixJoin.of(".", serviceName -> AMAZON_PATTERN
        .matcher(String.valueOf(serviceName))
        .replaceAll("")
        .trim())
  );

  public static CharSequence spanName(final Request<?> awsRequest) {
    return CACHE.computeIfAbsent(getQualifiedName(awsRequest).toString(), key -> UTF8BytesString.create(SpanNaming
      .instance()
      .namingSchema()
      .cloud()
      .operationForRequest(
  ...
  public static CharSequence getQualifiedName(final Request<?> awsRequest) {
    return CLASS_NAME_CACHE.getQualifiedName(
        awsRequest.getOriginalRequest().getClass(),
        awsRequest.getServiceName()
    );
  }
}
