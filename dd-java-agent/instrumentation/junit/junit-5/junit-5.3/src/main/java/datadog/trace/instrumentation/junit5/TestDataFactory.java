...
public abstract class TestDataFactory {
  ...
  private static volatile Map<String, Function<TestDescriptor, TestIdentifier>> TEST_IDENTIFIER_FACTORY_BY_ENGINE_ID =
      Collections.emptyMap();
  private static volatile Map<String, Function<TestDescriptor, TestSourceData>> TEST_SOURCE_DATA_FACTORY_BY_ENGINE_ID =
      Collections.emptyMap();
  private static volatile Map<String, Predicate<TestDescriptor>> TEST_DESCRIPTOR_FILTER_BY_ENGINE_ID =
      Collections.emptyMap();
  ...
  public static synchronized void register(
    ...
    TEST_IDENTIFIER_FACTORY_BY_ENGINE_ID = addEntry(
        TEST_IDENTIFIER_FACTORY_BY_ENGINE_ID,
        engineId,
        testIdentifierFactory
    );
    TEST_SOURCE_DATA_FACTORY_BY_ENGINE_ID = addEntry(
        TEST_SOURCE_DATA_FACTORY_BY_ENGINE_ID,
        engineId,
        testSourceDataFactory
    );
    if (testDescriptorFilter != null) {
      TEST_DESCRIPTOR_FILTER_BY_ENGINE_ID = addEntry(
          TEST_DESCRIPTOR_FILTER_BY_ENGINE_ID,
          engineId,
          testDescriptorFilter
      );
    }
  }
