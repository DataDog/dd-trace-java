...
/**
 * SparkJava uses static singletons for routing setup, which means tests
 * that modify the server state must run in a forked JVM to avoid interference.
 * This forked test ensures isolation for the SparkJava instrumentation tests.
 */
class SparkJavaForkedTest extends HttpServerTest<Object> {
  ...
  class SparkJavaServer implements HttpServer {
    ...
    SparkJavaServer() {
      ...
      Spark.post(CREATED.path) { req, res ->
        controller(CREATED) {
