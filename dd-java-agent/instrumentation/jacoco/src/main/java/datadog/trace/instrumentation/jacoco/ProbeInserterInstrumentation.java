...
public class ProbeInserterInstrumentation extends Instrumenter.CiVisibility implements Instrumenter.ForTypeHierarchy {
  ...
  public String[] helperClassNames() {
    return new String[] {
        packageName + ".ReflectiveMethodVisitor"
    };
  }
  ...
  public ElementMatcher<TypeDescription> hierarchyMatcher() {
    // The jacoco javaagent jar that is published relocates internal classes to an "obfuscated" package name
  ...
  public void adviceTransformations(AdviceTransformation transformation) {
    transformation.applyAdvice(isMethod().and(named("visitMaxs")), getClass().getName() + "$VisitMaxsAdvice");
    transformation.applyAdvice(isMethod().and(not(isStatic())).and(named("insertProbe")), getClass().getName() + "$InsertProbeAdvice");
  }
  ...
  public static class InsertProbeAdvice {
    ...
    static void exit(
        ...
        @Advice.Argument(0) final int id) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException, NoSuchFieldException {
