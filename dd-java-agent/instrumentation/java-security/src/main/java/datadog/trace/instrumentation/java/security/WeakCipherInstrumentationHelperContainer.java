...
public class WeakCipherInstrumentationHelperContainer implements InvokeDynamicHelperContainer {

  @InvokeDynamicHelper
  public static void onCipherAlgorithm(@Nullable final String algorithm) {
    if (algorithm == null) {
      return;
    }
    ...
    if (!Config.get().getIastWeakCipherAlgorithms().matcher(algorithmId).matches()) {
      return;
    }
