...
class VertxSqlClientForkedTest extends AgentTestRunner {
  @AutoCleanup
  @Shared
  // This database name must match up with the name in the CircleCI PG Docker definition
  ...
  def "test #type"() {
    ...
    assertTraces(1) {
      trace(3, true) {
        ...
        checkDBSpan(it, span(2), "SELECT ?", "SELECT", dbs.DBInfos.postgresql, prepared)
        basicSpan(it, "parent")
      }
    }
  ...
  def "test #type without parent"() {
    ...
    assertTraces(2) {
      trace(1) {
        checkDBSpan(it, null, "SELECT ?", "SELECT", dbs.DBInfos.postgresql, prepared)
      }
      trace(1) {
  ...
  def "test cursor"() {
    ...
    assertTraces(1) {
      trace(3, true) {
        basicSpan(it, "handler", span(2))
        checkDBSpan(it, span(2), "SELECT ?", "SELECT", dbs.DBInfos.postgresql, true)
        basicSpan(it, "parent")
      }
    }
  ...
  def "test row stream"() {
    ...
    assertTraces(1) {
      trace(3, true) {
        basicSpan(it, "handler", span(2))
        checkDBSpan(it, span(2), "SELECT ?", "SELECT", dbs.DBInfos.pos, true)
        basicSpan(it, "parent")
      }
    }
