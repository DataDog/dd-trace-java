...
import datadog.trace.instrumentation.akkahttp106.AkkaHttpClientDecorator
...
abstract class AkkaHttpScalaClientInstrumentationTest extends AkkaHttpClientInstrumentationTest {
  ...
  CompletionStage<HttpResponse> doRequest(HttpRequest request) {
    ...
    Future<akka.http.scaladsl.model.HttpResponse> f = http.singleRequest(sRequest, http.defaultClientHttpsContext(),
      (ConnectionPoolSettings) ConnectionPoolSettings.apply(system), system.log())
    return FutureConverters.toJava(f)
  }
}

class AkkaHttpJavaClientInstrumentationV0Test extends AkkaHttpJavaClientInstrumentationTest {
...
class AkkaHttpScalaClientInstrumentationV0Test extends AkkaHttpScalaClientInstrumentationTest {
