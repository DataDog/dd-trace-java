...
public class VertxTestServer extends AbstractVerticle {
  ...
  public void start(final Promise<Void> startPromise) {
    ...
    router
      ...
      .handler(ctx -> controller(ctx, BODY_MULTIPART, () -> {
        ctx.fileUploads();
        String res = convertFormAttributes(ctx);
    ...
    router.route(SESSION_ID.getPath()).handler(SessionHandler.create(LocalSessionStore.create(vertx)
    ));
    router
      .route(SESSION_ID.getPath())
      .handler(ctx -> ctx
        .response()
        .setStatusCode(SESSION_ID.getStatus())
        .end(ctx.session().id()));
    ...
    vertx
      ...
      .listen(port)
      .andThen(event -> {
        // send this though event bus and succeed deploy after successful response
        int actualPort = event.result().actualPort();
        vertx
          .eventBus()
          .request(PORT_DATA_ADDRESS, actualPort)
          .andThen(ar -> {
            if (ar.succeeded()) {
