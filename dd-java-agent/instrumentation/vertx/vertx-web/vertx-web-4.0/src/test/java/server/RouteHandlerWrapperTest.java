...
class RouteHandlerWrapperTest {
  ...
  void updateRouteWritesRouteToBothSpans() {
    TestRoutingContext context =
        new TestRoutingContext("/items/123", HttpMethod.GET).route("/items/:id");
  ...
  void updateRouteDoesNotReplaceRootRouteWhenOneExists() {
    TestRoutingContext context = new TestRoutingContext("/", HttpMethod.GET).route("/");
    AgentSpan parentSpan = mock(AgentSpan.class);
    AgentSpan handlerSpan = mock(AgentSpan.class);
  ...
  private static final class TestRoutingContext implements InvocationHandler {
    ...
    private TestRoutingContext(String requestPath, HttpMethod method) {
      this.instance = proxy(RoutingContext.class, this);
      this.request = request(requestPath, method);
    }
    ...
    private static HttpServerRequest request(String path, HttpMethod httpMethod) {
      return proxy(
