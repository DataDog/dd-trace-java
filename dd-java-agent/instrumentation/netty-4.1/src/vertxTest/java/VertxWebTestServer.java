...
public class VertxWebTestServer extends AbstractVerticle {
  ...
  public static Vertx start(final int port) throws ExecutionException, InterruptedException {
    ...
    vertx.deployVerticle(new VertxWebTestServer(port), res -> {
      if (!res.succeeded()) {
  ...
  public void start() {
    ...
    router.route("/error").handler(routingContext -> {
      routingContext.response().setStatusCode(500).end();
    });
    router.route("/test").handler(routingContext -> {
      // FIXME: we probably would want to make things like 'vertx.setTimeout' work as expected too
      tracedMethod();
      routingContext.response().putHeader("content-type", "text/html").end("Hello World");
    });
