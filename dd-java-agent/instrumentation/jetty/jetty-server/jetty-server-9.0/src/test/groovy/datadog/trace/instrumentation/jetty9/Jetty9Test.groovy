...
abstract class Jetty9Test extends HttpServerTest<Server> {
  ...
  def 'test blocking of multipart and urlencoded request body is pinned after block-telemetry-3 wiring #variant'() {
    ...
    // (jetty-appsec-8.1.3's multipart path is disabled for this Jetty 9.0.x module, see
    // testBodyMultipart() above) after the block-telemetry-3 changes wired the reportBlockFailure()
    // call into these advice classes. reportBlockFailure() itself remains unreachable through this
    // end-to-end test: the real JettyBlockingHelper.tryCommitBlockingResponse always returns true
    // for a genuine attempt (see .claude-invariants.md), so no test here can force that branch.
    response.code() == 413
