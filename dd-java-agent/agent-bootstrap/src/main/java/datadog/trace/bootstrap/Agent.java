...
public class Agent {
  ...
  private static void maybeStartIast(Class<?> scoClass, Object o) {
    ...
    InstrumentationGateway gw = AgentTracer.get().instrumentationGateway();
    startIast(gw, scoClass, o);
  }

  private static void startIast(InstrumentationGateway gw, Class<?> scoClass, Object sco) {
    try {
      final Class<?> iastSysClass = IAST_CLASSLOADER.loadClass("com.datadog.iast.IastSystem");
      final Method iastInstallerMethod =
          iastSysClass.getMethod("start", InstrumentationGateway.class);
      iastInstallerMethod.invoke(null, gw, sco);
    } catch (final Throwable ex) {
