...
/**
 * The buffering half of RUM injection for byte streams: a circular lookbehind buffer plus draining,
 * committing, byte counting and the actual content injection. It knows nothing about <em>where</em>
 * to inject; a {@link ByteMatcher} decides that by driving these operations.
 *
 ...
 */
@NotThreadSafe
public final class InjectingByteBuffer {
  ...
  public InjectingByteBuffer(
    ...
    this.lookbehind = new byte[lookbehindSize];
    this.pos = 0;
    this.count = 0;
  ...
  public void passthrough(int b) throws IOException {
    if (wasDraining) {
      drain();
    }
    downstream.write(b);
  ...
  public void passthrough(byte[] array, int off, int len) throws IOException {
    if (wasDraining) {
      drain();
    }
  ...
  /** Writes a range of the source array straight downstream, counting it as original bytes. */
  public void writeThrough(byte[] array, int off, int len) throws IOException {
    downstream.write(array, off, len);
    bytesWritten += len;
  }

  /** Holds a unit in the lookbehind buffer, spilling the oldest downstream if it is full. */
  public void hold(int b) throws IOException {
