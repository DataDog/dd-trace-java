 ...
 * {@link InjectingCharBuffer}.
 */
@NotThreadSafe
public final class HtmlCharMatcher implements CharMatcher {
  ...
  @Override
  public void write(int c, InjectingCharBuffer buffer) throws IOException {
    if (!buffer.isFiltering()) {
      buffer.passthrough(c);
      return;
    }
    apply(head.accept(c), c, buffer);
  }

  @Override
  public void write(char[] array, int off, int len, InjectingCharBuffer buffer) throws IOException {
    if (!buffer.isFiltering()) {
    ...
    while (i < end) {
      ...
      apply(head.accept(array[i]), array[i], buffer);
      i++;
      if (!buffer.isFiltering()) {
  ...
  private static void apply(final int action, final int unit, final InjectingCharBuffer buffer)
      throws IOException {
    switch (action) {
