...
public final class SessionState {
  private static final AtomicReferenceFieldUpdater<SessionState, MessageBatchState> BATCH_STATE = AtomicReferenceFieldUpdater.newUpdater(
      SessionState.class,
      MessageBatchState.class,
      "batchState");
  ...
  public boolean isAutoAcknowledge() {
    return ackMode != 0 && ackMode != 2;
    /* treat all other modes as Session.AUTO_ACKNOWLEDGE */
    ...
    if (null != capturedSpans) {
      synchronized (capturedSpans) {
        if (isClientAcknowledge() && implicitMessageAck()) {
          // time-in-queues spans have a different clean-up mechanism
          finishCapturedSpans();
        }
  ...
  private boolean implicitMessageAck() {
    ...
    if (oldestCaptureTime == 0) {
      // will be cleared when finishCapturedSpans is called
    ...
    while (itr.hasNext()) {
      ...
      if (!entry.getKey().isAlive()) {
        ...
        if (oldestEntries.size() > MIN_EVICTED_THREADS) {
          // discard the youngest span to keep the oldest N spans
          oldestEntries.poll();
        }
      }
    }
