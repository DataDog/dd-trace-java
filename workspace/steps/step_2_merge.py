...
Reads:
- configurations_descriptions_step_2_extracted.json
- step_2_overrides.json
...
def main() -> int:
    ...
    ap.add_argument(
        "--step-2-extracted",
        default="./workspace/result/configurations_descriptions_step_2_extracted.json",
    )
    ap.add_argument(
        "--step-2-overrides",
        default="./workspace/result/step_2_overrides.json",
    )
    ap.add_argument("--output", default="./workspace/result")
    args = ap.parse_args()

    extracted_path = Path(args.step_2_extracted).resolve()
    overrides_path = Path(args.step_2_overrides).resolve()
    out_dir = Path(args.output).resolve()

    eprint(f"[step2-merge] extracted={extracted_path}")
    eprint(f"[step2-merge] overrides={overrides_path}")
    eprint(f"[step2-merge] output-dir={out_dir}")

    extracted = read_json(extracted_path)
    if not isinstance(extracted, dict):
        raise ValueError("Extracted step 2 file is not a JSON object")
    ...
    for item in add_results:
        ...
        if res.get("source") != SOURCE_DOC:
            raise ValueError(f"Step 2 override addResults for {k} {v} must have source={SOURCE_DOC}")
        if not isinstance(res.get("description"), str) or not str(res.get("description")).strip():
            raise ValueError(f"Step 2 override addResults for {k} {v} must include non-empty description")
        if not isinstance(res.get("sourceFile"), str) or not str(res.get("sourceFile")).strip():
            raise ValueError(f"Step 2 override addResults for {k} {v} must include non-empty sourceFile")
    ...
    eprint(f"[step2-merge] applied addResults={applied}")
    ...
    out_path = out_dir / "configurations_descriptions_step_2.json"
    write_json(out_path, out_obj)
    eprint(
        f"[step2-merge] wrote {out_path} (documented={len(new_documented)} missing={len(new_missing)})"
    )
    return 0
