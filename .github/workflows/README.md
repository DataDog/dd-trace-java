...
* Pass the `merge-gate` status check on pull requests so they remain in a mergeable state,
* When a pull request is enqueued in GitHub merge queue, post a `/merge` comment to trigger the Datadog merge queue,
* Fail the `merge-gate` status check on merge groups to prevent GitHub from merging directly.
