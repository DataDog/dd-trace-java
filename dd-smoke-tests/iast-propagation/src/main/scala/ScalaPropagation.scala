...
class ScalaPropagation extends Supplier[util.List[String]] {
  ...
  /** Test case to reproduce IllegalFormatConversionException from original stack trace. Root cause:
    * Scala's format() calls unwrapArg() which converts BigDecimal -> Double. IAST receives
    * BigDecimal without unwrapping, causing IllegalFormatConversionException.
    */
