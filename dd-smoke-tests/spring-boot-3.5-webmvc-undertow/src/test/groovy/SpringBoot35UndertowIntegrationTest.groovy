...
class SpringBoot35UndertowIntegrationTest extends AbstractServerSmokeTest {
  ...
  def "responds to /fruits without hanging"() {
    setup:
  ...
  def "responds to /fruits/{name} without hanging"() {
    setup:
  ...
  def "actuator health endpoint remains responsive"() {
    setup:
    String url = "http://localhost:${httpPort}/actuator/health"
    ...
    then:
    response.code() == 200
    response.body().string().contains("UP")
  }
  ...
  List<String> expectedTelemetryDependencies() {
    ['spring-core', 'io.undertow:undertow-core']
  }
}
