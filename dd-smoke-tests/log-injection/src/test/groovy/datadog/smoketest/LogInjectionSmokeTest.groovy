...
abstract class LogInjectionSmokeTest extends AbstractSmokeTest {
  ...
  ProcessBuilder createProcessBuilder() {
    ...
    logCapture = jarName.endsWith("LogCapture")
    if (logCapture) {
      jarName = jarName.substring(0, jarName.length() - "LogCapture".length())
    }
    ...
    if (supportsLogCapture()) {
      command.add("-Ddd.$GeneralConfig.LOGS_CAPTURE_ENABLED=true" as String)
    }
  ...
  private assertParsedJsonLinesWithLogCapture(List<Map> logLines, String firstTraceId, String firstSpanId, String secondTraceId, String secondSpanId, String thirdTraceId, String thirdSpanId, String forthTraceId, String forthSpanId) {
    assert logLines.every {
      getFromContext(it, "dd.service") == noTags ? null : SERVICE_NAME
    }
    assert logLines.every {
  ...
  def "check raw file injection"() {
    ...
    if (supportsLogCapture()) {
      ...
      assertParsedJsonLinesWithLogCapture(lines, firstTraceId, firstSpanId, secondTraceId, secondSpanId, thirdTraceId, thirdSpanId, forthTraceId, forthSpanId)
    }
  }
...
class Slf4jInterfaceLogbackBackendLogCapture extends Slf4jInterfaceLogbackBackend {
  @Override
  def supportsDirectLogSubmission() {
    false
  }
...
class Slf4jInterfaceLog4j2BackendLogCapture extends Slf4jInterfaceLog4j2Backend {
  @Override
  def supportsDirectLogSubmission() {
    false
  }
