 ...
 * /v0.7/config} poll, and read back the tracer's poll requests (to assert the products it
 * subscribes to and the capabilities it advertises).
 */
public final class RemoteConfig {
  ...
  /**
   * Pushes a Remote Configuration payload the app's tracer will receive on its next {@code
   * /v0.7/config} poll.
   *
  ...
  /**
   * Waits (up to the default timeout) for a captured Remote Config poll request satisfying the
   * given predicate, and returns it.
   *
  ...
  /**
   * As {@link #waitForRequest(Predicate)}, but overriding the timeout for this call.
   *
   * @param predicate The predicate a poll request must satisfy.
