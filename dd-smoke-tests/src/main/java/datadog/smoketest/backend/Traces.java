...
/**
 * Query facade over the traces a {@link AgentBackend} has received. It is backend-agnostic: it
 * reads a live snapshot of decoded traces, so the same assertions run against either backend.
 */
public final class Traces {
   ...
   * received. Traces arrive asynchronously from the app, so callers wait for the expected count
   * before asserting structure.
   *
   ...
   * absorbs the async arrival of traces from a separately-launched app:
   *
   * <pre>{@code
  ...
  // Trace-invariant checks (ENABLED_CHECKS) are test-agent-specific — validated by that backend
  // itself (TestAgentBackend#assertNoInvariantFailures) rather than on this common, portable
  // facade.
}
