  ...
  public static void main(String[] args) throws Exception {
    ...
    RatpackServer.start(server -> server
      ...
      .handlers(chain -> chain
        .path("api_security/sampling/:status_code", ctx -> {
          ctx
            .getResponse()
            .status(Integer.parseInt(ctx.getPathTokens().get("status_code")))
            .send("EXECUTED");
        })
        .path("api_security/response", ctx -> ctx.parse(JsonNode.class).then(node -> {
