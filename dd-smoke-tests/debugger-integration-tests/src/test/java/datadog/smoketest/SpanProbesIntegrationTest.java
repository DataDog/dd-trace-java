...
public class SpanProbesIntegrationTest extends SimpleAppDebuggerIntegrationTest {
  ...
  void testMethodSpan() throws Exception {
    ...
    // 3 statuses + 1 for letting the trace being sent (async)
    ...
    processRequests(() -> statusResult.get() && traceReceived.get(), () -> String.format(
  ...
  void testSingleLineSpan() throws Exception {
    final String METHOD_NAME = "fullMethod";
    // 2 probe statuses: RECEIVED + ERROR
    final String EXPECTED_UPLOADS = "2";
    SpanProbe spanProbe = SpanProbe
      .builder()
      .probeId(PROBE_ID)
