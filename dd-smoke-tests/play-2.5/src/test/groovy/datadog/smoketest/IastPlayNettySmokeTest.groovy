...
class IastPlayNettySmokeTest extends AbstractIastServerSmokeTest {
  ...
  void 'Test that the server starts'() {
    expect:
    def request = new Request.Builder()
      .url("http://localhost:${httpPort}/iast/multiple_vulns/1/?param=value1")
      .get()
      .build()
    def response = client.newCall(request as Request).execute()
    assert response.code() == 200, "Response code was ${response.code()} for request ${request}"
  }
  ...
  //    when:
  //    requests.each { req ->
  //      def response = client.newCall(req as Request).execute()
  //      assert response.code() == 200, "Response code was ${response.code()} for request ${req}"
  //    }
  //
  ...
  //    hasVulnerability { vul -> vul.type == 'WEAK_HASH' && vul.location.method == 'multipleVulns' && vul.evidence.value == 'MD5'}
  //    hasVulnerability { vul -> vul.type == 'WEAK_HASH' && vul.location.method == 'multipleVulns' && vul.evidence.value == 'RIPEMD128'}
  //}
