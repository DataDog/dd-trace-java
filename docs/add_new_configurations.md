...
7. Add the Environment Variable name of the configuration to the `supportedConfigurations` key of `metadata/supported-configurations.json`.
   1. The key is the Environment Variable name, and the value is an array of objects with the following fields:
      ...
      6. Sensitive. This is an _optional_ boolean field that defaults to `false`. Set it to `true` for credentials or other secret values that must be redacted from configuration telemetry. Aliases inherit the setting.
    ...
    "DD_APPLICATION_KEY": [
      {
        ...
        "aliases": ["DD_APP_KEY"], // Mandatory, allows empty array if no aliases exist
        "sensitive": true // Optional, defaults to false and applies to aliases
      }
    ],
