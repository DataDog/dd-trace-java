...
public final class PaddedSequence extends LongRhsPadding {
  ...
  static {
    try {
      VALUE_HANDLE =
          MethodHandles.lookup().findVarHandle(PaddedSequence.class, "value", long.class);
    } catch (Throwable t) {
   ...
   * message-passing.
   *
   * @return the current sequence value with acquire ordering
   */
   ...
   *
   * @return the current sequence value with no memory barrier
   */
  ...
  /**
   * Atomically updates the value to {@code newValue} if the current value equals {@code expected},
   * but may fail spuriously even if the current value equals {@code expected}.
   *
   * <p>This is a <strong>weak CAS</strong> operation. Weak CAS is intrinsified in modern JVMs and
   * generally faster than strong CAS. It is suitable for usage inside CAS loops, where the loop
   * retries until success.
   *
   * <p><strong>Important:</strong> This method may return {@code false} spuriously, even if no
   * other thread has modified the value. Do <em>not</em> rely on weak CAS for single-shot atomic
   * updates that must succeed or fail deterministically.
   *
   * @param expected the value the field must currently hold
   * @param newValue the new value to set
   * @return {@code true} if the CAS succeeded, {@code false} otherwise (may be spurious)
   */
  public boolean weakCompareAndSet(long expected, long newValue) {
    return VALUE_HANDLE.weakCompareAndSet(this, expected, newValue);
  }
}
