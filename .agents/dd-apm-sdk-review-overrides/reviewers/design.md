...
- `products/` modules typically follow the `{product}-api` / `{product}-bootstrap` / `{product}-lib` / `{product}-agent` layering, but no existing product implements it exactly: `metrics` has no `-bootstrap`; `feature-flagging` adds an extra `-config` submodule. Don't flag a missing or extra submodule name against this list — the layering shape is aspirational, not enumerable. Don't flag implementation that already matches a product's documented role: `feature-flagging-api` is the OpenFeature provider (evaluators / hooks); `feature-flagging-config` loads configuration. A hard finding is only a dependency, bootstrap-safety, or public-API boundary break that **ARCHITECTURE.md** actually states — not "this could have gone in `-lib`."
...
The bootstrap and advice correctness rules for this code live in **AGENTS.md § "Critical constraints"** and **docs/bootstrap_design_guidelines.md** / **docs/instrumentation_design_guidelines.md** — this lens owns them; do not restate them from memory, open the doc and check the diff against it. (The performance override's "Bootstrap / startup-latency note" covers the same code from the cost angle — that's a different finding on the same lines, not a duplicate.) Respect the ordering in ARCHITECTURE.md § "Startup Sequence": `AgentBootstrap.premain()` must stay tiny and side-effect-free; anything heavier belongs in `Agent.start()` or a product's own `*System.start()`. Those methods run during the JVM premain phase — that is the intended placement, not a violation. Do not reject established `Agent.start()` / `*System.start()` work as "premain-reachable." Classes that lock in a JUL / NIO / JMX implementation (`java.util.logging.*`, `java.nio.file.*`, `javax.management.*`) must not run in any premain phase (very-early `AgentBootstrap.premain()` or later `Agent.start()`); defer that work until after `main`, per **docs/bootstrap_design_guidelines.md**.
