...
**Concrete failure (Cassandra regen, R-DB-1):** the eval was given the integration slug `cassandra`, but dd-trace-java's family directory is `datastax-cassandra/` with siblings `datastax-cassandra-3.0/`, `-3.8/`, `-4.0/`, all declaring `super("cassandra")`. The agent created a new top-level `instrumentation/cassandra/` module — a SECOND module instrumenting the same Cassandra 4.x types as the family member it should have restored. Result: a **silent tracing outage** — two modules' advice targeting the same types collided (mutual suppression via the shared call-depth guard), zero spans, all tests timed out, and there was no build error to catch it. (The shared `super("cassandra")` name is a *symptom* that flagged the duplication, not the mechanism — modules are indexed by class, so equal names alone are legal; the harm is the duplicate module advising the same types.) This is especially dangerous under the blind protocol: if the same-version master module was deleted, "modify it in place" has no target — but the surviving siblings still hold the name, so grepping for the name (not looking for a same-version directory) is what tells you where the module belongs. When the name is taken and the correct family directory differs from the slug you were handed, place the module in the family directory and match the siblings' `super(...)` exactly; if there is genuinely no correct home without colliding, STOP and surface it rather than shipping a parallel registration.
