...
- Implement the **narrowest** `Instrumenter` interface possible:
  ...
  - **RPC-client-and-server design rule:** when a library has distinct client and server call paths (gRPC, Thrift, Dubbo, any request/response protocol with two roles), give each role its own decorator/component pair — do not share one `Decorator`/`component` constant across both. A client span and a server span for the same call must carry distinct `component` values (e.g. `grpc-client` vs `grpc-server`), distinct `span.kind` (`client` vs `server`), and independently-computed status/error tags — collapsing them into one shared decorator silently loses the role distinction that lets a trace viewer tell "we called out" from "we were called."
...
**Rewriting a module wholesale — treat the pre-existing file set as a floor, not a suggestion.** When a task asks you to rework or regenerate a module rather than edit it, list every pre-existing file in the module directory before writing anything, and account for each one — either point to its successor in your new layout, or explicitly note that you're dropping it and why. A file that shares no obvious class with whatever you're focused on (a profiling hook, a rarely-hit edge-case advice class, a legacy compatibility shim) is exactly the kind you'll otherwise never reconsider, because nothing in your working set naturally leads you back to it. Silently losing that file is not narrowing scope on purpose — it's an omission that a diff review only catches by noticing a deletion with no corresponding addition. *Worked example:* a gRPC module rewrite focused on the client/server call-path classes (`ClientCallImpl`, `ServerCallImpl` and their advice) legitimately absorbed several old per-class advice files into fewer, broader interceptor classes — but also silently dropped an unrelated Netty write-queue profiling hook (`QueuedCommandInstrumentation`) that shared no class with anything the rewrite was reasoning about. The fix wasn't to avoid consolidating advice — it was to explicitly enumerate every file being replaced, including the ones outside the immediate focus, before calling the rewrite done.
