...
**Before assuming a reactive API needs a separate instrumentation, trace whether it shares the eager path's chokepoint.** Some clients wire reactive and eager APIs onto the same low-level operation path (e.g. a shared wire-protocol `CommandListener`, or a driver where both surfaces ultimately call one internal method) — one instrumentation on that shared point covers both, no split needed. Others expose a reactive path (`connection.reactive()`, `AbstractRedisReactiveCommands`, or similar) that builds a lazy `Mono`/`Flux`/`Publisher` running only on subscription, on a different call path than the eager method — hooking the eager method alone misses these. Trace an actual reactive call from its public API entry point down to where a span would be created; if it reaches the same method your eager advice already hooks, no separate instrumentation is needed. If it diverges onto its own path, add a separate instrumentation there and migrate the span across the subscription lifecycle. Either way, only ship a reactive test once you've confirmed which case applies — a reactive test with no matching instrumentation coverage will assert spans that don't exist and fail in CI.
