...
**A reactive API is a SEPARATE instrumentation from the eager/synchronous one — hooking one does not cover the other.** Many clients expose an eager path (a `dispatch`/`send`/`execute` method that runs the operation immediately or enqueues it) AND a reactive path (`connection.reactive()`, `AbstractRedisReactiveCommands`, or similar) that builds a lazy `Mono`/`Flux`/`Publisher` and only runs the operation on subscription, often on another thread. Advice on the eager chokepoint produces **zero spans for the reactive path** — the reactive command never passes through it. To cover reactive, add a separate instrumentation that hooks the reactive-command creation and migrates the span across the subscription lifecycle (see the reactive-streams / `Mono`/`Flux` propagation patterns for the library). If you are only covering the eager path, do NOT ship a test that exercises the reactive API and asserts spans — it will assert spans the instrumentation cannot produce and fail in CI. Scope the tests to what the instrumentation actually covers, or add the reactive instrumentation.
