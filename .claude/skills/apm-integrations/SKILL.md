...
description: Write, modify, or extend a Datadog APM integration for dd-trace-java — instrument a third-party Java library so it produces traces, spans, and tags through the Datadog Java tracer. Use when the user wants to add a new APM integration, library instrumentation, ByteBuddy advice, instrumenter module, decorator, or contextstore for any Java library (HTTP clients, databases, messaging systems, web frameworks, RPC libraries, GraphQL servers, reactive streams, gRPC, JDBC drivers, etc.). Triggers: "add APM integration", "instrument library X", "write a Java instrumenter", "add tracing for", "ByteBuddy advice", "InstrumenterModule", "wrap method", "trace this client", "add span for", "muzzle directive", "dd-java-agent instrumentation", "add to the Java tracer", "create instrumentation module".
---
...
Before starting, familiarize yourself with these guides:

- **[ByteBuddy Patterns](references/bytebuddy-patterns.md)** - R1-R14 rules, Advice constraints, span lifecycle
- **[Reference Integrations](references/reference-integrations.md)** - Canonical examples by category
- **[Implementation Guide](references/implementation-guide.md)** - Step-by-step walkthrough with code examples
- **[Anti-Patterns](references/anti-patterns.md)** - Common mistakes and how to avoid them

## Step 1 – Read the authoritative docs

Before writing any code, read the dd-trace-java documentation:
...
**Read [Reference Integrations](references/reference-integrations.md)** to find the canonical example for your category.

Search `dd-java-agent/instrumentation/` for a structurally similar integration:
- Same target system (HTTP client, database, messaging, etc.)
...
**See [Implementation Guide](references/implementation-guide.md)** for complete file structure and build.gradle template.
...
**CRITICAL**: Read **[ByteBuddy Patterns](references/bytebuddy-patterns.md)** for complete R1-R14 rules before writing Advice code.
...
- Use `CallDepthThreadLocalMap` to guard against recursive instrumentation of the same method (R5)
...
**See [Anti-Patterns](references/anti-patterns.md)** for common mistakes and debugging tips.
...
**If muzzle fails:** check for missing helper class names in `helperClassNames()` (R4, R11).

**If tests fail:** verify span lifecycle order (start → activate → error → finish → close), helper registration,
and `contextStore()` map entries match actual usage. See [ByteBuddy Patterns](references/bytebuddy-patterns.md) for R6-R8.

**If spotlessCheck fails:** run `./gradlew spotlessApply` to auto-format (R14), then re-check.
