...
- **Inlined advice does not appear under `datadog.trace.instrumentation.<name>.*`** — ByteBuddy `@Advice` is inlined into the host class. Allocation/CPU caused by advice shows up under the host-class frame (`java.sql.*`, `org.h2.*`, `test.Test*`, etc.). The 4-bucket analyzer accounts for this.
...
- **Per-call ns includes JIT compilation amortized over the loop.** Not steady-state.
