 ...
 */
public final class AdaptiveSampler8 extends AdaptiveSampler {
  ...
  public AdaptiveSampler8(
    ...
    this(
        Duration.of(
            TimeUnit.NANOSECONDS.convert(windowDuration, windowDurationUnit), ChronoUnit.NANOS),
        samplesPerWindow,
        lookback,
  ...
  @Override
  public boolean keep() {
    final AdaptiveSampler8.Counts counts = countsRef.get();
    counts.addTest();
    counts.addSample();
    return true;
  }
  ...
  private static class RollWindowTask implements Task<AdaptiveSampler8> {

    static final RollWindowTask INSTANCE = new RollWindowTask();

    @Override
    public void run(final AdaptiveSampler8 target) {
      target.rollWindow();
    }
  }
