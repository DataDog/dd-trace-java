...
/**
 * <ul>Benchmark comparing different approaches to filling and reading a Map in a multi-thread context.
 ...
 * 
 * <p>If a Map can be created atomically in one thread and then stored into a volatile, 
 * that is the preferred solution.  For example, requesting an update from agent / API and then 
 * exposing to the rest of the tracer via a global.
 * 
 * <p>If a Map needs to be written in a thread-safe manner, but is primarily accessed from one thread at a time,
 * then a synchronized HashMap is usually the best option.
 * 
 * <code>
...
public class ThreadSafeMapBenchmark {
  ...
  static final ConcurrentSkipListMap<String, Integer> CONC_SKIP_LIST_MAP = _create_concSkipListMap();
