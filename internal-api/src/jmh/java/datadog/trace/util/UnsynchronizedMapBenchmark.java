...
/**
 * <ul>Benchmark comparing different Map-s...
 * <li>(RECOMMENDED) HashMap - for fastest lookups - (not typically needed for tags)
 * <li>(RECOMMENDED) TagMap - for storing tags - especially if copying between maps or using builders
 * <li>TreeMap - better for custom Comparators - case-insensitive Maps (see CaseInsensitiveMapBenchmark)
 ...
 * <p>HashMap is 2x faster throughput-wise to create and has less memory overhead because 
 * there's no linked list to capture insertion order.
 * 
 * <p>TreeMap is useful when a custom Comparator is needed -- see CaseInsensitiveMapBenchmark
 * 
 * <p>HashMap & TagMap also perform exceedingly well in cases where the exact same object is used for 
 * put & get operations.  e.g. when using String literals or Class literals as keys
 * 
 * <code>
...
public class UnsynchronizedMapBenchmark {
  ...
  static final String[] EQUAL_KEYS = init(() -> {
  ...
  static Map<String, Integer> _create_hashMap_sized() {
	// Sizing is preferable for large maps, but in practice, most of our maps typically fall within the default
  ...
  public void iterate_tagMap_forEach(Blackhole blackhole) {
	...
	TAG_MAP.forEach(blackhole, (bh, entry) -> {
