...
public interface AgentSpanEvent {
  ...
  /**
   * Serializes the event as a single JSON object, matching the legacy v0.x {@code events} tag
   * representation (for example {@code {"time_unix_nano":..,"name":"..","attributes":{..}}}).
   *
   * @return The event as a JSON object.
   */
  CharSequence toJsonTag();
}
