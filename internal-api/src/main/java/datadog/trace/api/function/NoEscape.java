...
/**
 * Marks a type that must never be <em>retained</em> -- never assigned to a field, put into a
 ...
 * constraint to readers and to give a future checker something to verify -- that no field (instance
 * or static) is declared with a {@code @NoEscape} type. The discipline it names is <b>not yet
 * enforced</b>; hold to it by hand until the checker lands.
 *
 * <p><b>On a type</b> ({@link ElementType#TYPE}): instances of this type must never be stored in a
 * field or collection. Returning one, passing it to a callback, or chaining further calls on it is
 * fine -- what is not fine is anything that keeps it alive past the operation using it.
 *
 ...
 * checks.md}) -- without needing to read this class's prose above.
 *
 * <ul>
 ...
 *       rule flags <em>storage</em> that outlives the call, not ordinary use within it.
 ...
 *       (e.g. call {@code toString()}) before storing it.
