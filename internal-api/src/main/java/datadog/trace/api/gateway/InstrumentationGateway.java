      ...
      case USER_ID:
        return (C) new BiFunction<RequestContext, String, Flow<Void>>() {
          ...
          public Flow<Void> apply(RequestContext ctx, String userId) {
            try {
              return ((BiFunction<RequestContext, String, Flow<Void>>) callback).apply(ctx, userId);
            } catch (Throwable t) {
              log.warn("Callback for {} threw.", eventType, t);
              return Flow.ResultFlow.empty();
            }
    ...
    switch (eventType.getId()) {
      case REQUEST_ENDED_ID:
        return (C) (BiFunction<RequestContext, IGSpanInfo, Flow<Void>>) (ctx, agentSpan) -> {
          Flow<Void> flowAppSec = ((BiFunction<RequestContext, IGSpanInfo, Flow<Void>>)
                  callbackAppSec)
              .apply(ctx, agentSpan);
          Flow<Void> flowIast = ((BiFunction<RequestContext, IGSpanInfo, Flow<Void>>) callbackIast)
              .apply(ctx, agentSpan);
